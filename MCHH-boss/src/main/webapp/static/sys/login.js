...
function submit()
{
	...
	$("input[nullmsg]").each(function() {
	   if ($('#userName').val() == "") {
			showError($('#userName').attr("nullmsg"), 500);
			jrumble();
		...
		if ($('#password').val() == "") {
			showError($('#password').attr("nullmsg"), 500);
			jrumble();
			setTimeout('hideTop()', 1000);
			submit = false;
			return false;
...
function Login() {
	...
	var formData = 'password='+$('#md5password').val()+'&'+'loginName='+$('#userName').val();
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				loginsuccess();
				setTimeout("window.location.href='"+checkurl+"'", 1000);
			} else {
				showError('用户名或密码错误!');
			}
		}
