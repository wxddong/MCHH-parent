...
<html>
  <head>
    ...
    <link href="${ctx}/static/css/first_prenatalListEdit_styles.css" type="text/css" rel="stylesheet"/>
    ...
    <script type="text/javascript">
    	...
    	$(function(){
			...
			$("#u478").click(function(){
				...
				var addUrl = "${ctx}/prenatal/listEditFirstPrenatalExa"
				if(ba == 1){
					return false;
				}
			...
			$("#u480").click(function(){
				...
				var finishUrl = "${ctx}/prenatal/editFirstExa";
			...
			//专案管理
			$("#u482").click(function(){
				var zUrl = "${ctx}/com/config/highrisk/firstEditRedioHighRisk";
				var prenatalId = $("#prenatalId").val();
				//alert(prenatalId);
				$.ajax({
   					url:zUrl,
   					...
   					success:function(data){
   						...
   						//$("#checkRadio").html(cStr);
   						//$("#u05_input").val()
						showDialog_l("提示","addZhuan","moremiddle",false);
   					}
   				});
			...
			var vc = $("#u443_input").attr("checked");
			if(vc){
			...
			if(operation == null ||operation == ""){
				$("#u203_input")[0].selectedIndex = -1;
			}
			var allergy = $("#allergy").val();
			if(allergy == null ||allergy == ""){
				$("#u205_input")[0].selectedIndex = -1;
			}
			var heart = $("#heart").val();
			if(heart == null ||heart == ""){
				$("#u231_input")[0].selectedIndex = -1;
			}
			var lung = $("#lung").val();
			if(lung == null ||lung == ""){
				$("#u233_input")[0].selectedIndex = -1;
			}
			var outerVulva = $("#outerVulva").val();
			if(outerVulva == null ||outerVulva == ""){
				$("#u249_input")[0].selectedIndex = -1;
			}
			var vagina = $("#vagina").val();
			if(vagina == null ||vagina == ""){
				$("#u251_input")[0].selectedIndex = -1
			}
			var uterus = $("#uterus").val();
			if(uterus == null ||uterus == ""){
				$("#u253_input")[0].selectedIndex = -1;
			}
			var uterineNeck = $("#uterineNeck").val();
			if(uterineNeck == null ||uterineNeck == ""){
				$("#u255_input")[0].selectedIndex = -1;
			}
			var appendix = $("#appendix").val();
			if(appendix == null ||appendix == ""){
				$("#u257_input")[0].selectedIndex = -1;
			}
			...
			//页面加载计算bmi值
			var high = $("#u219_input").val();
			/* if(high == ""){
				alert("请输入身高！");
			} */
			var weight = $("#u224_input").val();
			if(high != "" && high != null && weight != null && weight != ""){
				high = high / 100;
			...
			var funtNum = $("#u473_input").val();
			if(funtNum == null || funtNum == ""){
				$("#u472_input")[0].selectedIndex = -1;
			}
    	...
    	function only(){
    		if($("#u472_input").val()=="0"){
    			$("#u473_input").val("0");
    			$("#u473_input")[0].readOnly=true;
    		}else{
    			$("#u473_input").val("");
    			$("#u473_input")[0].readonly=false;
    		}
    	};
    	/**
    	 * @author dingjh
    ...
    <form id="form" action="" method="post">
		<div id="title" class="ax_形状" style="background-color: #FFFFFF;border: 1px solid #CCCCCC;">
            ...
            <c:if test="${map.correctCount != '0' }">
	            <div id="u481" class="ax_形状" <c:if test="${map.birthArchives.riskSign != '1' }">style="display: none"</c:if>>
	          		<p id="u482" style="cursor:pointer;"><span>专案管理</span></p>
	        	</div>
        	</c:if>
        	<c:if test="${map.correctCount == '0' }">
        		<div id="u481" class="ax_形状" style="display: none">
	          		<p id="u482" style="cursor:pointer;"><span>专案管理</span></p>
	        	</div>
        		<div id="u4810" class="ax_形状" style="background-color:#E9854F;" >
          			<p id="u4820" onclick="window.location.href='${ctx}/highriskprojectmanagement/list?birthArchiveId=${map.birthArchives.id }&archivesId=${map.birthArchives.archivesId }'" style="cursor:pointer;"><span>专案记录</span></p>
        		</div>
        	</c:if>
  ...
  font-weight:700;"</c:if>>
            ...
            <div id="u30" class="ax_文本段落">
            	<input type="hidden" name="ct" value="<fmt:formatDate value="${map.prenatalExa.creatTime }" pattern="yyyy-MM-dd"/>"/>
          		<p id="u31"><span>检查日期：
          		<c:if test="${map.prenatalExa.normalSign == '1' }">
          		<fmt:formatDate value="${map.prenatalExa.creatTime }" pattern="yyyyMMdd"/>
        		...
        		class="" value="<fmt:formatDate value="${map.prenatalExa.creatTime }" pattern="yyyyMMdd"/>" tabindex="1" required>
        		</c:if>
          		<c:if test="${map.prenatalExa.normalSign == '0' }">
        ...
        <div id="" name="foldDiv2" style="height: 329px">
            ...
            <div id="u61" class="ax_文本段落">
          	...
          	<div id="u63" class="ax_文本框"><p style="position: absolute;top: 6px;" id="week">${map.motherhood.pregnancyWeeks }</p>
          		<input id="u63_input" name="pregnancyWeeks" type="hidden" value="${map.motherhood.pregnancyWeeks }"/>
          	</div>
          	<div id="u64" class="ax_文本段落">
           	  <p id="u65"><span>周</span></p>
          	</div>
          	<div id="u74" class="ax_文本框"><p style="position: absolute;top: 6px;" id="day">${map.motherhood.pregnancyDays }</p>
            	<input id="u74_input" name="pregnancyDays" type="hidden" value="${map.motherhood.pregnancyDays }"/>
            </div>
            ...
            <div id="u75" class="ax_下拉列表框"><p style="position: absolute;top: 5px;"><fmt:formatDate value="${map.motherhood.lastMenses }" pattern="yyyy-MM-dd"/></p>
        		<input type="hidden" id="u75_input" name="lastMenses" readonly 
        style="color:#333333;cursor:pointer" onfocus="WdatePicker({dateFmt:'yyyyMMdd'})" 
        class="Wdate" value="<fmt:formatDate value="${map.motherhood.lastMenses }" pattern="yyyy-MM-dd"/>" tabindex="1" required>
      		</div>
            
            <div id="u44" class="ax_文本段落">
          		<p id="u45"><span>预产期： </span><span id="birthTime"><fmt:formatDate value="${map.expectedDate }" pattern="yyyy-MM-dd"/></span></p>
      		</div>
            ...
            <div id="u48" class="ax_下拉列表框">
        		<select id="u48_input" <c:if test="${map.motherhood.pregnancyNum == null }">class="selt"</c:if> name="pregnancyNum">
                    <option <c:if test="${map.motherhood.pregnancyNum == '0'}">selected</c:if> value="0">0</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '1'}">selected</c:if> value="1">1</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '2'}">selected</c:if> value="2">2</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '3'}">selected</c:if> value="3">3</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '4'}">selected</c:if> value="4">4</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '5'}">selected</c:if> value="5">5</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '6'}">selected</c:if> value="6">6</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '7'}">selected</c:if> value="7">7</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '8'}">selected</c:if> value="8">8</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '9'}">selected</c:if> value="9">9</option>
                    <option <c:if test="${map.motherhood.pregnancyNum == '10'}">selected</c:if> value="10">10</option>
        		</select>
      		</div>
            ...
            <div id="u68" class="ax_下拉列表框">
        		<select id="u68_input" <c:if test="${map.motherhood.bearNum == null }">class="selt"</c:if> name="bearNum">
                    <option <c:if test="${map.motherhood.bearNum == '0'}">selected</c:if> value="0">0</option>
                    <option <c:if test="${map.motherhood.bearNum == '1'}">selected</c:if> value="1">1</option>
                    <option <c:if test="${map.motherhood.bearNum == '2'}">selected</c:if> value="2">2</option>
                    <option <c:if test="${map.motherhood.bearNum == '3'}">selected</c:if> value="3">3</option>
        		</select>
     		</div>
            ...
            <div id="u51" class="ax_下拉列表框">
        		<select id="u51_input" name="eutociaNum" <c:if test="${map.motherhood.eutociaNum == null }">class="selt"</c:if>>
                    <option <c:if test="${map.motherhood.eutociaNum == '0'}">selected</c:if> value="0">0</option>
                    <option <c:if test="${map.motherhood.eutociaNum == '1'}">selected</c:if> value="1">1</option>
                    <option <c:if test="${map.motherhood.eutociaNum == '2'}">selected</c:if> value="2">2</option>
                    <option <c:if test="${map.motherhood.eutociaNum == '3'}">selected</c:if> value="3">3</option>
                    <option <c:if test="${map.motherhood.eutociaNum == '4'}">selected</c:if> value="4">4</option>
        		</select>
      		</div>
            ...
            <div id="u52" class="ax_下拉列表框">
        		<select id="u52_input" name="cesareanNum" <c:if test="${map.motherhood.cesareanNum == null }">class="selt"</c:if>>
                    <option <c:if test="${map.motherhood.cesareanNum == '0'}">selected</c:if> value="0">0</option>
                    <option <c:if test="${map.motherhood.cesareanNum == '1'}">selected</c:if> value="1">1</option>
                    <option <c:if test="${map.motherhood.cesareanNum == '2'}">selected</c:if> value="2">2</option>
                    <option <c:if test="${map.motherhood.cesareanNum == '3'}">selected</c:if> value="3">3</option>
        		</select>
      		</div>
          ...
          <div id="u78" class="ax_下拉列表框">
       		<select id="u78_input" name="prebearNum" <c:if test="${map.motherhood.prebearNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.prebearNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.prebearNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.prebearNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.prebearNum == '3'}">selected</c:if> value="3">3</option>
                  <option <c:if test="${map.motherhood.prebearNum == '4'}">selected</c:if> value="4">4</option>
                  <option <c:if test="${map.motherhood.prebearNum == '5'}">selected</c:if> value="5">5</option>
                  <option <c:if test="${map.motherhood.prebearNum == '6'}">selected</c:if> value="6">6</option>
                  <option <c:if test="${map.motherhood.prebearNum == '7'}">selected</c:if> value="7">7</option>
                  <option <c:if test="${map.motherhood.prebearNum == '8'}">selected</c:if> value="8">8</option>
                  <option <c:if test="${map.motherhood.prebearNum == '9'}">selected</c:if> value="9">9</option>
                  <option <c:if test="${map.motherhood.prebearNum == '10'}">selected</c:if> value="10">10</option>
       		  </select>
       		</div>
            ...
            <div id="u87" class="ax_下拉列表框">
                <select id="u87_input" name="naturalAbortionNum" <c:if test="${map.motherhood.naturalAbortionNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.naturalAbortionNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.naturalAbortionNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.naturalAbortionNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.naturalAbortionNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u81" class="ax_下拉列表框">
                <select id="u81_input" name="inducedAbortionNum" <c:if test="${map.motherhood.inducedAbortionNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.inducedAbortionNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.inducedAbortionNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.inducedAbortionNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.inducedAbortionNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u82" class="ax_下拉列表框">
                <select id="u82_input" name="drugAbortionNum" <c:if test="${map.motherhood.drugAbortionNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.drugAbortionNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.drugAbortionNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.drugAbortionNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.drugAbortionNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u96" class="ax_下拉列表框">
                <select id="u96_input" name="exfetationNum" <c:if test="${map.motherhood.exfetationNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.exfetationNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.exfetationNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.exfetationNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.exfetationNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u101" class="ax_下拉列表框">
                <select id="u101_input" name="inducelabourNum" <c:if test="${map.motherhood.inducelabourNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.inducelabourNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.inducelabourNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.inducelabourNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.inducelabourNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u116" class="ax_下拉列表框">
                <select id="u116_input" name="abortionNum" <c:if test="${map.motherhood.abortionNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.abortionNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.abortionNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.abortionNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.abortionNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u110" class="ax_下拉列表框">
            	<select name="deadfetusNum" id="u110_input" <c:if test="${map.motherhood.deadfetusNum == null }">class="selt"</c:if>>
	                <option <c:if test="${map.motherhood.deadfetusNum == '0'}">selected</c:if> value="0">0</option>
	                <option <c:if test="${map.motherhood.deadfetusNum == '1'}">selected</c:if> value="1">1</option>
	                <option <c:if test="${map.motherhood.deadfetusNum == '2'}">selected</c:if> value="2">2</option>
	                <option <c:if test="${map.motherhood.deadfetusNum == '3'}">selected</c:if> value="3">3</option>
      			</select>
            </div>
            ...
            <div id="u111" class="ax_下拉列表框">
                <select id="u111_input" name="stillbirthNum" <c:if test="${map.motherhood.stillbirthNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.stillbirthNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.stillbirthNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.stillbirthNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.stillbirthNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u123" class="ax_下拉列表框">
                <select id="u123_input" name="newbornDieNum" <c:if test="${map.motherhood.newbornDieNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.newbornDieNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.newbornDieNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.newbornDieNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.newbornDieNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u128" class="ax_下拉列表框">
              <select id="u128_input" name="defectNum" <c:if test="${map.motherhood.defectNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.defectNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.defectNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.defectNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.defectNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u141" class="ax_下拉列表框">
                <select id="u141_input" name="bleedingNum" <c:if test="${map.motherhood.bleedingNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.bleedingNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.bleedingNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.bleedingNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.bleedingNum == '3'}">selected</c:if> value="3">3</option>
                </select>
            </div>
			...
			<div id="u135" class="ax_下拉列表框">
                <select id="u135_input" name="twinsNum" <c:if test="${map.motherhood.twinsNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.twinsNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.twinsNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.twinsNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.twinsNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
			...
			<div id="u136" class="ax_下拉列表框">
                <select id="u136_input" name="deformityNum" <c:if test="${map.motherhood.deformityNum == null }">class="selt"</c:if>>
                  <option <c:if test="${map.motherhood.deformityNum == '0'}">selected</c:if> value="0">0</option>
                  <option <c:if test="${map.motherhood.deformityNum == '1'}">selected</c:if> value="1">1</option>
                  <option <c:if test="${map.motherhood.deformityNum == '2'}">selected</c:if> value="2">2</option>
                  <option <c:if test="${map.motherhood.deformityNum == '3'}">selected</c:if> value="3">3</option>
                </select>
      		</div>
            ...
            <div id="u155" class="ax_复选框">
                ...
                <input id="u155_input" <c:if test="${history == '0' }">checked="checked"</c:if> type="checkbox" name="history" value="0"/>
          	</div>
          	</c:forEach>
          	<c:forEach items="${map.historys }" var = "history">
            <div id="u157" class="ax_复选框">
                ...
                <input id="u157_input" <c:if test="${history == '1' }">checked="checked"</c:if> type="checkbox" name="history" value="1"/>
      		</div>
      		</c:forEach>
          	<c:forEach items="${map.historys }" var = "history">
            <div id="u159" class="ax_复选框">
                ...
                <input id="u159_input" <c:if test="${history == '2' }">checked="checked"</c:if> type="checkbox" name="history" value="2"/>
      		</div>
      		</c:forEach>
          	<c:forEach items="${map.historys }" var = "history">
            <div id="u189" class="ax_复选框">
                ...
                <input id="u189_input" <c:if test="${history == '3' }">checked="checked"</c:if> type="checkbox" name="history" value="3"/>
      		</div>
      		</c:forEach>
          	<c:forEach items="${map.historys }" var = "history">
            <div id="u161" class="ax_复选框">
                ...
                <input id="u161_input" <c:if test="${history == '4' }">checked="checked"</c:if> type="checkbox" name="history" value="4"/>
      		</div>
      		</c:forEach>
          	<c:forEach items="${map.historys }" var = "history">
            <div id="u163" class="ax_复选框">
                ...
                <input id="u163_input" <c:if test="${history == '5' }">checked="checked"</c:if> type="checkbox" name="history" value="5"/>
      		</div>
      		</c:forEach>
          	<c:forEach items="${map.historys }" var = "history">
            <div id="u165" class="ax_复选框">
                ...
                <input id="u165_input" <c:if test="${history == '6' }">checked="checked"</c:if> type="checkbox" name="history" value="6"/>
      		</div>
      		</c:forEach>
          	<c:forEach items="${map.historys }" var = "history">
            <div id="u167" class="ax_复选框">
                ...
                <input id="u167_input" <c:if test="${history == '7' }">checked="checked"</c:if> type="checkbox" name="history" value="7"/>
      		</div>
      		</c:forEach>
            <div id="u73" class="ax_文本框">
        		<input id="u73_input" <c:if test="${history != '7' }">style="display: none;"</c:if> type="text" name="historyOthers" placeholder="其他时显示输入框" value="${map.motherhood.historyOthers }"/>
      		</div>
            ...
            <div id="u191" class="ax_复选框">
                ...
                <input id="u191_input" <c:if test="${famHistory == '0' }">checked="checked"</c:if> name="famHistory" type="checkbox" value="0"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.famHistorys }" var="famHistory">
            <div id="u171" class="ax_复选框">
                ...
                <input id="u171_input" <c:if test="${famHistory == '1' }">checked="checked"</c:if> name="famHistory" type="checkbox" value="1"/>
      		</div>
            <div id="u489" class="ax_文本框">
        		<input id="u489_input" name="heredityRelation" type="text" placeholder="与患者的关系" value="${map.motherhood.heredityRelation }"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.famHistorys }" var="famHistory">
            <div id="u173" class="ax_复选框">
                ...
                <input id="u173_input" <c:if test="${famHistory == '2' }">checked="checked"</c:if> name="famHistory" type="checkbox" value="2"/>
      		</div>
            <div id="u490" class="ax_文本框">
        		<input id="u490_input" name="mentalRelation" type="text" placeholder="与患者的关系" value="${map.motherhood.mentalRelation }"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.famHistorys }" var="famHistory">
            <div id="u198" class="ax_复选框">
                ...
                <input id="u198_input" <c:if test="${famHistory == '3' }">checked="checked"</c:if> name="famHistory" type="checkbox" value="3"/>
      		</div>
      		</c:forEach>
            <div id="u197" class="ax_文本框">
        		<input id="u197_input" <c:if test="${famHistory != '3' }">style="display: none;"</c:if> type="text" name="famHistoryOthers" placeholder="其他时显示输入框" value="${map.motherhood.famHistoryOthers }"/>
      		</div>
            ...
            <div id="u193" class="ax_复选框">
                ...
                <input id="u193_input" <c:if test="${perHistory == '0' }">checked="checked"</c:if> name="perHistory" type="checkbox" value="0"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.perHistorys }" var="perHistory">
            <div id="u175" class="ax_复选框">
                ...
                <input id="u175_input" <c:if test="${perHistory == '1' }">checked="checked"</c:if> name="perHistory" type="checkbox" value="1"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.perHistorys }" var="perHistory">
            <div id="u177" class="ax_复选框">
                ...
                <input id="u177_input" <c:if test="${perHistory == '2' }">checked="checked"</c:if> name="perHistory" type="checkbox" value="2"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.perHistorys }" var="perHistory">
            <div id="u179" class="ax_复选框">
                ...
                <input id="u179_input" <c:if test="${perHistory == '3' }">checked="checked"</c:if> name="perHistory" type="checkbox" value="3"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.perHistorys }" var="perHistory">
            <div id="u181" class="ax_复选框">
                ...
                <input id="u181_input" <c:if test="${perHistory == '4' }">checked="checked"</c:if> name="perHistory" type="checkbox" value="4"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.perHistorys }" var="perHistory">
            <div id="u183" class="ax_复选框">
                ...
                <input id="u183_input" <c:if test="${perHistory == '5' }">checked="checked"</c:if> name="perHistory" type="checkbox" value="5"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.perHistorys }" var="perHistory">
            <div id="u201" class="ax_复选框">
                ...
                <input id="u201_input" <c:if test="${perHistory == '6' }">checked="checked"</c:if> name="perHistory" type="checkbox" value="6"/>
      		</div>
      		</c:forEach>
            <div id="u200" class="ax_文本框">
        		<input id="u200_input" <c:if test="${perHistory != '6' }">style="display: none;"</c:if> name="perHistoryOthers" type="text" placeholder="其他时显示输入框" value="${map.motherhood.perHistoryOthers }"/>
      		</div>
            ...
            <div id="u203" class="ax_下拉列表框">
            	<input type="hidden" id="operation" value="${map.motherhood.operation}"/>
                <select id="u203_input" class="test" name="operation">
                  <option <c:if test="${map.motherhood.operation == '0' }">selected</c:if> value="0">无</option>
                  <option <c:if test="${map.motherhood.operation == '1' }">selected</c:if> value="1">有</option>
                </select>
      		</div>
            <div id="u204" class="ax_文本框">
        		<input id="u204_input" type="text" class="u203_input" <c:if test="${map.motherhood.operation != '1' }">style="display: none;"</c:if> name="operationRemark" placeholder="其他时显示输入框" value="${map.motherhood.operationRemark }"/>
      		</div>
            ...
            <div id="u205" class="ax_下拉列表框">
            	<input type="hidden" value="${map.motherhood.allergy }" id="allergy"/>
                <select id="u205_input" class="test" name="allergy">
                  <option value="0" <c:if test="${map.motherhood.allergy == '0' }">selected</c:if>>无</option>
                  <option value="1" <c:if test="${map.motherhood.allergy == '1' }">selected</c:if>>有</option>
                </select>
      		</div>
            <div id="u206" class="ax_文本框">
        		<input id="u206_input" type="text" class="u205_input" <c:if test="${map.motherhood.allergy != '1' }">style="display: none;"</c:if> name="allergyRemark" placeholder="有时显示输入框" value="${map.motherhood.allergyRemark }"/>
      		</div>
        ...
        <div id="" name="foldDiv4" style="height: 78px">
            ...
            <div id="u219" class="ax_文本框">
        		<input id="u219_input" name="height" type="text" value="${map.bloodStethos.height }"/>
      		</div>
            ...
            <div id="u224" class="ax_文本框">
        		<input id="u224_input" name="weight" type="text" value="${map.bloodStethos.weight }"/>
      		</div>
            ...
            <div id="u227" class="ax_文本框">
        		<input id="u227_input" name="bloodHi" type="text" value="${map.bloodStethos.bloodHi }"/>
      		</div>
            <div id="u228" class="ax_文本段落">
          		<p id="u229"><span>/</span></p>
      		</div>
            <div id="u230" class="ax_文本框">
        		<input id="u230_input" name="bloodLow" type="text" value="${map.bloodStethos.bloodLow }"/>
      		</div>
            ...
            <div id="u231" class="ax_下拉列表框">
            	<input type="hidden" value="${map.bloodStethos.heart}" id="heart"/>
                <select id="u231_input" class="test" name="heart">
                  <option <c:if test="${map.bloodStethos.heart == '0' }">selected</c:if> value="0">未见异常</option>
                  <option <c:if test="${map.bloodStethos.heart == '1' }">selected</c:if> value="1">异常</option>
                </select>
      		</div>
            <div id="u232" class="ax_文本框">
        		<input id="u232_input" type="text" class="u231_input" <c:if test="${map.bloodStethos.heart != '1' }">style="display: none;" </c:if> name="heartRemark" placeholder="异常时显示输入框" value="${map.bloodStethos.heartRemark }"/>
      		</div>
            ...
            <div id="u233" class="ax_下拉列表框">
            	<input type="hidden" id="lung" value="${map.bloodStethos.lung}"/>
                <select id="u233_input" class="test" name="lung">
                  <option <c:if test="${map.bloodStethos.lung == '0' }">selected</c:if> value="0">未见异常</option>
                  <option <c:if test="${map.bloodStethos.lung == '1' }">selected</c:if> value="1">异常</option>
                </select>
      		</div>
            <div id="u234" class="ax_文本框">
        		<input id="u234_input" name="lungRemark" class="u233_input" <c:if test="${map.bloodStethos.lung != '1' }"> style="display: none;"</c:if> type="text" placeholder="异常时显示输入框" value="${map.bloodStethos.lungRemark }"/>
      		</div>
        ...
        <div id="" name="foldDiv6" style="height: 111px">
            ...
            <div id="u249" class="ax_下拉列表框">
            	<input type="hidden" value="${map.gynecologcal.outerVulva}" id="outerVulva"/>
                <select id="u249_input" class="test1" name="outerVulva">
                  <option <c:if test="${map.gynecologcal.outerVulva == '0' }">selected</c:if> value="0">未见异常</option>
                  <option <c:if test="${map.gynecologcal.outerVulva == '2' }">selected</c:if> value="2">未检查</option>
                  <option <c:if test="${map.gynecologcal.outerVulva == '1' }">selected</c:if> value="1">异常</option>
                </select>
      		</div>
            <div id="u250" class="ax_文本框">
        		<input id="u250_input" name="outerVulvaRemark" class="u249_input" <c:if test="${map.gynecologcal.outerVulva != 1}"> style="display: none;"</c:if> type="text" placeholder="异常时显示输入框" value="${map.gynecologcal.outerVulvaRemark }"/>
      		</div>
            ...
            <div id="u251" class="ax_下拉列表框">
            	<input type="hidden" value="${map.gynecologcal.vagina}" id="vagina"/>
                <select id="u251_input" class="test1" name="vagina">
                  <option <c:if test="${map.gynecologcal.vagina == '0' }">selected</c:if> value="0">未见异常</option>
                  <option <c:if test="${map.gynecologcal.vagina == '2' }">selected</c:if> value="2">未检查</option>
                  <option <c:if test="${map.gynecologcal.vagina == '1' }">selected</c:if> value="1">异常</option>
                </select>
      		</div>
            <div id="u252" class="ax_文本框">
        		<input id="u252_input" name="vaginaRemark" class="u251_input" <c:if test="${map.gynecologcal.vagina != '1' }"> style="display: none;"</c:if> type="text" placeholder="异常时显示输入框" value="${map.gynecologcal.vaginaRemark }"/>
      		</div>
            ...
            <div id="u253" class="ax_下拉列表框">
            	<input type="hidden" value="${map.gynecologcal.uterus}" id="uterus"/>
                <select id="u253_input" class="test1" name="uterus">
                  <option <c:if test="${map.gynecologcal.uterus == '0' }">selected</c:if> value="0">未见异常</option>
                  <option <c:if test="${map.gynecologcal.uterus == '2' }">selected</c:if> value="2">正常孕期</option>
                  <option <c:if test="${map.gynecologcal.uterus == '3' }">selected</c:if> value="3">未检查</option>
                  <option <c:if test="${map.gynecologcal.uterus == '1' }">selected</c:if> value="1">异常</option>
                </select>
      		</div>
            <div id="u254" class="ax_文本框">
        		<input id="u254_input" name="uterusRemark" class="u253_input" <c:if test="${map.gynecologcal.uterus != '1' && map.gynecologcal.uterus != '2'}">style="display: none;" </c:if> type="text" placeholder="异常时显示输入框" value="${map.gynecologcal.uterusRemark }"/>
      		</div>
            ...
            <div id="u255" class="ax_下拉列表框">
            	<input type="hidden" value="${map.gynecologcal.uterineNeck}" id="uterineNeck"/>
                <select id="u255_input" class="test1" name="uterineNeck" <c:if test="${map.gynecologcal.uterineNeck == null }">class="selt"</c:if>>
                  <option <c:if test="${map.gynecologcal.uterineNeck == '0' }">selected</c:if> value="0">未见异常</option>
                  <option <c:if test="${map.gynecologcal.uterineNeck == '2' }">selected</c:if> value="2">未检查</option>
                  <option <c:if test="${map.gynecologcal.uterineNeck == '1' }">selected</c:if> value="1">异常</option>
                </select>
      		</div>
            <div id="u256" class="ax_文本框">
        		<input id="u256_input" type="text" class="u255_input" <c:if test="${map.gynecologcal.uterineNeck != '1' }"> style="display: none;"</c:if> name="uterineNeckRemark" placeholder="异常时显示输入框" value="${map.gynecologcal.uterineNeckRemark }"/>
      		</div>
            ...
            <div id="u257" class="ax_下拉列表框">
            	<input type="hidden" value="${map.gynecologcal.appendix}" id="appendix"/>
                <select id="u257_input" class="test1" name="appendix" <c:if test="${map.gynecologcal.appendix == null }">class="selt"</c:if>>
                  <option <c:if test="${map.gynecologcal.appendix == '0' }">selected</c:if> value="0">未见异常</option>
                  <option <c:if test="${map.gynecologcal.appendix == '2' }">selected</c:if> value="2">未检查</option>
                  <option <c:if test="${map.gynecologcal.appendix == '1' }">selected</c:if> value="1">异常</option>
                </select>
      		</div>
            <div id="u258" class="ax_文本框">
        		<input id="u258_input" name="appendixRemark" class="u257_input" <c:if test="${map.gynecologcal.appendix != '1' }"> style="display: none;"</c:if> type="text" placeholder="异常时显示输入框" value="${map.gynecologcal.appendixRemark }"/>
      		</div>
        ...
        <div id="" name="foldDiv8" style="height: 563px">
            ...
            <div id="u358" class="ax_文本框">
        		<input id="u358_input" name="hemoglobin" type="text" value="${map.supplemetnaryExam.hemoglobin}"/>
     		</div>
            <div id="u265" class="ax_文本段落">
          		<p id="u266"><span style="font-family:'Arial Normal', 'Arial';font-weight:400;color:#333333;">g/L</span><span style="font-family:'Arial Normal', 'Arial';font-weight:400;color:#333333;">&nbsp; </span><span style="font-family:'Arial Negreta', 'Arial';font-weight:700;color:#FF0000;"></span></p>
      		</div>
            ...
            <div id="u359" class="ax_文本框">
        		<input id="u359_input" name="leucocyte" type="text" value="${map.supplemetnaryExam.leucocyte}"/>
      		</div>
            ...
            <div id="u360" class="ax_文本框">
        		<input id="u360_input" name="platelet" type="text" value="${map.supplemetnaryExam.platelet}"/>
      		</div>
            ...
            <div id="u355" class="ax_文本框">
        		<input id="u355_input" name="bloodOthers" type="text" value="${map.supplemetnaryExam.bloodOthers}"/>
      		</div>
            ...
            <div id="u377" class="ax_文本框">
        		<input id="u377_input" name="urineSugar" type="text" value="${map.supplemetnaryExam.urineSugar}"/>
      		</div>
            ...
            <div id="u378" class="ax_文本框">
        		<input id="u378_input" name="ketone" type="text" value="${map.supplemetnaryExam.ketone}"/>
      		</div>
            ...
            <div id="u379" class="ax_文本框">
        		<input id="u379_input" name="proteinuria" type="text" value="${map.supplemetnaryExam.proteinuria}"/>
      		</div>
            ...
            <div id="u380" class="ax_文本框">
        		<input id="u380_input" name="urineBlood" type="text" value="${map.supplemetnaryExam.urineBlood}"/>
      		</div>
            ...
            <div id="u381" class="ax_文本框">
        		<input id="u381_input" name="urineOthers" type="text" value="${map.supplemetnaryExam.urineOthers}"/>
      		</div>
            ...
            <div id="u384" class="ax_下拉列表框">
                <select id="u384_input" name=bloodType <c:if test="${map.supplemetnaryExam.bloodType == null }">class="selt"</c:if>>
                  <option <c:if test="${map.supplemetnaryExam.bloodType == '0'}">selected</c:if> value="0">A型</option>
                  <option <c:if test="${map.supplemetnaryExam.bloodType == '1'}">selected</c:if> value="1">B型</option>
                  <option <c:if test="${map.supplemetnaryExam.bloodType == '2'}">selected</c:if> value="2">AB型</option>
                  <option <c:if test="${map.supplemetnaryExam.bloodType == '3'}">selected</c:if> value="3">O型</option>
                </select>
      		</div>
            ...
            <div id="u385" class="ax_下拉列表框">
                <select id="u385_input" name="bloodTypeRh" <c:if test="${map.supplemetnaryExam.bloodTypeRh == null }">class="selt"</c:if>>
                  <option <c:if test="${map.supplemetnaryExam.bloodTypeRh == '0'}">selected</c:if> value="0">RH+</option>
                  <option <c:if test="${map.supplemetnaryExam.bloodTypeRh == '1'}">selected</c:if> value="1">RH-</option>
                </select>
      		</div>
            ...
            <div id="u390" class="ax_文本框">
        		<input id="u390_input" name="bloodGlucose" type="text" value="${map.supplemetnaryExam.bloodGlucose}"/>
      		</div>
            ...
            <div id="u401" class="ax_文本框">
        		<input id="u401_input" name="sgpt" type="text" value="${map.supplemetnaryExam.sgpt}"/>
      		</div>
            ...
            <div id="u402" class="ax_文本框">
        		<input id="u402_input" name="sgot" type="text" value="${map.supplemetnaryExam.sgot}"/>
      		</div>
            ...
            <div id="u403" class="ax_文本框">
        		<input id="u403_input" name="albumin" type="text" value="${map.supplemetnaryExam.albumin}"/>
      		</div>
            ...
            <div id="u404" class="ax_文本框">
        		<input id="u404_input" name="totalBilirubin" type="text" value="${map.supplemetnaryExam.totalBilirubin}"/>
      		</div>
            ...
            <div id="u405" class="ax_文本框">
        		<input id="u405_input" name="dbil" type="text" value="${map.supplemetnaryExam.dbil}"/>
      		</div>
            ...
            <div id="u406" class="ax_文本框">
        		<input id="u406_input" name="scr" type="text" value="${map.supplemetnaryExam.scr}"/>
      		</div>
            ...
            <div id="u407" class="ax_文本框">
        		<input id="u407_input" name="bun" type="text" value="${map.supplemetnaryExam.bun}"/>
      		</div>
            ...
            <div id="u301" class="ax_复选框">
                ...
                <input id="u301_input" <c:if test="${vagin == '0'}">checked="checked"</c:if> name="vaginalFluid" type="checkbox" value="0"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.vaginalFluids }" var="vagin">
            <div id="u303" class="ax_复选框">
                ...
                <input id="u303_input" <c:if test="${vagin == '1'}">checked="checked"</c:if> name="vaginalFluid" type="checkbox" value="1"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.vaginalFluids }" var="vagin">
            <div id="u305" class="ax_复选框">
                ...
                <input id="u305_input" <c:if test="${vagin == '2'}">checked="checked"</c:if> name="vaginalFluid" type="checkbox" value="2"/>
      		</div>
      		</c:forEach>
      		<c:forEach items="${map.vaginalFluids }" var="vagin">
            <div id="u409" class="ax_复选框">
                ...
                <input id="u409_input" <c:if test="${vagin == '3'}">checked="checked"</c:if> name="vaginalFluid" type="checkbox" value="3"/>
      		</div>
      		</c:forEach>
            <div id="u408" class="ax_文本框">
        		<input id="u408_input" <c:if test="${vagin != '3'}">style="display: none;"</c:if>  name="vaginalFluidOthers" type="text" placeholder="其他时显示输入框" value="${map.supplemetnaryExam.vaginalFluidOthers}"/>
      		</div>
            ...
            <div id="u351" class="ax_下拉列表框">
                <select id="u351_input" name="vaginalClean" <c:if test="${map.supplemetnaryExam.vaginalClean == null }">class="selt"</c:if>>
                  <option <c:if test="${map.supplemetnaryExam.vaginalClean == '0'}">selected</c:if> value="0">Ⅰ度</option>
                  <option <c:if test="${map.supplemetnaryExam.vaginalClean == '1'}">selected</c:if> value="1">Ⅱ度 </option>
                  <option <c:if test="${map.supplemetnaryExam.vaginalClean == '2'}">selected</c:if> value="2">Ⅲ度</option>
                  <option <c:if test="${map.supplemetnaryExam.vaginalClean == '3'}">selected</c:if> value="3"> Ⅳ度</option>
                </select>
      		</div>
            ...
            <div id="u327" class="ax_单选按钮">
                ...
                <input id="u327_input" <c:if test="${map.supplemetnaryExam.hbsag == '0'}">checked="checked"</c:if> type="radio" value="0" name="hbsag"/>
      		</div>
            <div id="u329" class="ax_单选按钮">
                <label for="u329_input">
                    <p id="u330"><span>阳性</span></p>
                </label>
                <input id="u329_input" <c:if test="${map.supplemetnaryExam.hbsag == '1'}">checked="checked"</c:if> type="radio" value="1" name="hbsag"/>
      		</div>
            ...
            <div id="u331" class="ax_单选按钮">
                ...
                <input id="u331_input" <c:if test="${map.supplemetnaryExam.hbsab == '0'}">checked="checked"</c:if> type="radio" value="0" name="hbsab"/>
      		</div>
            <div id="u333" class="ax_单选按钮">
                <label for="u333_input">
                    <p id="u334"><span>阳性</span></p>
                </label>
                <input id="u333_input" <c:if test="${map.supplemetnaryExam.hbsab == '1'}">checked="checked"</c:if> type="radio" value="1" name="hbsab"/>
      		</div>
            ...
            <div id="u339" class="ax_单选按钮">
                ...
                <input id="u339_input" <c:if test="${map.supplemetnaryExam.hbeag == '0'}">checked="checked"</c:if> type="radio" value="0" name="hbcab"/>
      		</div>
            <div id="u341" class="ax_单选按钮">
                <label for="u341_input">
                    <p id="u342"><span>阳性</span></p>
                </label>
                <input id="u341_input" <c:if test="${map.supplemetnaryExam.hbeag == '1'}">checked="checked"</c:if> type="radio" value="1" name="hbcab"/>
      		</div>
            ...
            <div id="u335" class="ax_单选按钮">
                ...
                <input id="u335_input" <c:if test="${map.supplemetnaryExam.hbeab == '0'}">checked="checked"</c:if> type="radio" value="0" name="hbeab"/>
      		</div>
            <div id="u337" class="ax_单选按钮">
                    <label for="u337_input">
                        <p id="u338"><span>阳性</span></p>
                    </label>
                    <input id="u337_input" <c:if test="${map.supplemetnaryExam.hbeab == '1'}">checked="checked"</c:if> type="radio" value="1" name="hbeab"/>
      		</div>
            ...
            <div id="u343" class="ax_单选按钮">
                ...
                <input id="u343_input" <c:if test="${map.supplemetnaryExam.hbcab == '0'}">checked="checked"</c:if> type="radio" value="0" name="hbeag"/>
      		</div>
            <div id="u345" class="ax_单选按钮">
                <label for="u345_input">
                    <p id="u346" ><span>阳性</span></p>
                </label>
                <input id="u345_input" <c:if test="${map.supplemetnaryExam.hbcab == '1'}">checked="checked"</c:if> type="radio" value="1" name="hbeag"/>
      		</div>
            ...
            <div id="u411" class="ax_单选按钮">
                ...
                <input id="u411_input" <c:if test="${map.supplemetnaryExam.syphilisSerology == '0'}">checked="checked"</c:if> type="radio" value="0" name="syphilisSerology"/>
      		</div>
            <div id="u413" class="ax_单选按钮">
                <label for="u413_input">
                    <p id="u414"><span>阳性</span></p>
                </label>
                <input id="u413_input" <c:if test="${map.supplemetnaryExam.syphilisSerology == '1'}">checked="checked"</c:if> type="radio" value="1" name="syphilisSerology"/>
      		</div>
            ...
            <div id="u415" class="ax_单选按钮">
                ...
                <input id="u415_input" <c:if test="${map.supplemetnaryExam.hivab == '0'}">checked="checked"</c:if> type="radio" value="0" name="hivab"/>
      		</div>
            <div id="u417" class="ax_单选按钮">
                <label for="u417_input">
                    <p id="u418"><span>阳性</span></p>
                </label>
                <input id="u417_input" <c:if test="${map.supplemetnaryExam.hivab == '1'}">checked="checked"</c:if> type="radio" value="1" name="hivab"/>
      		</div>
            <div id="u325" class="ax_文本段落">
          		<p id="u326"><span>B超：</span></p>
      		</div>
             <div id="u420" class="ax_文本框">
                <select id="u420_input" name="bscan" <c:if test="${map.supplemetnaryExam.bscan == null }">class="selt"</c:if>>
                  <option <c:if test="${map.supplemetnaryExam.bscan == '1'}">selected</c:if> value="1">无异常</option>
                  <option <c:if test="${map.supplemetnaryExam.bscan == '2'}">selected</c:if> value="2">有异常</option>
                  <option <c:if test="${map.supplemetnaryExam.bscan == '0'}">selected</c:if> value="0">未检查</option>
                </select>
      		</div>
            <div id="u42400">
            	<input id="u42400_input" <c:if test="${map.supplemetnaryExam.bscan != '2'}">style="display: none;"</c:if>  name="bscanRemark" type="text" placeholder="异常时显示输入框" value="${map.supplemetnaryExam.bscanRemark}"/>
       	  	</div>
            ...
            <div id="u354" class="ax_下拉列表框">
                <select id="u354_input" name="nt" <c:if test="${map.supplemetnaryExam.nt == null }">class="selt"</c:if>>
                  <option <c:if test="${map.supplemetnaryExam.nt == '1'}">selected</c:if> value="1">无异常</option>
                  <option <c:if test="${map.supplemetnaryExam.nt == '2'}">selected</c:if> value="2">有异常</option>
                  <option <c:if test="${map.supplemetnaryExam.nt == '0'}">selected</c:if> value="0">未检查</option>
                </select> 
      		</div>
            <div id="u419" class="ax_文本框">
        		<input id="u419_input" <c:if test="${map.supplemetnaryExam.nt != '2'}">style="display: none;"</c:if>  type="text" name="ntRemark" placeholder="异常时显示输入框" value="${map.supplemetnaryExam.ntRemark}"/>
      		</div>
      ...
      <div id="" name="foldDiv10" style="height: 43px">
        ...
        <div id="u427" class="ax_单选按钮">
            ...
            <input id="u427_input" <c:if test="${map.prenatalExa.evaluate == '0'}">checked</c:if> type="radio" value="0" name="evaluate"/>
      	</div>
        <div id="u429" class="ax_单选按钮">
            <label for="u429_input">
                <p id="u430"><span> 异常</span></p>
            </label>
            <input id="u429_input" <c:if test="${map.prenatalExa.evaluate == '1'}">checked</c:if> type="radio" value="1" name="evaluate"/>
      	</div>
      	
	        <div id="u431"<c:if test="${map.prenatalExa.evaluate != '1'}"> style="display: none;" </c:if>class="ax_下拉列表框">
	            <p id="u431_input" style="cursor:pointer;background-color: #EE5F45"><span>异常选项</span></p>
	      	</div>
      	
      	
	       <!--  <div id="u431" class="ax_下拉列表框">
	            <p id="u431_input" style="cursor:pointer;background-color: #EE5F45"><span>异常选项</span></p>
	      	</div> -->
      ...
      <div id="" name="foldDiv12" style="height: 171px">
        ...
        <div id="u436" class="ax_多行文本框">
        	<textarea id="u436_input" name="guidance">${map.prenatalExa.guidance }</textarea>
      	</div>
      ...
      <div id="" name="foldDiv14" style="height: 253px">
        ...
        <div id="u443" class="ax_单选按钮">
            ...
            <input id="u443_input" <c:if test="${map.prenatalReferral.referral == '0' }">checked="checked"</c:if> type="radio" value="0" name="referral"/>
      	</div>
        <div id="u445" class="ax_单选按钮">
            <label for="u445_input">
                <p id="u446"><span> 有</span></p>
            </label>
            <input id="u445_input" <c:if test="${map.prenatalReferral.referral == '1' }">checked="checked"</c:if> type="radio" value="1" name="referral"/>
      	</div>
        ...
        <div id="u449" class="ax_文本框">
        	<input id="u449_input" type="text" name="reason" value="${map.prenatalReferral.reason}"/>
      	</div>
        ...
        <div id="u452" class="ax_下拉列表框">
            ...
            <c:forEach items="${depts }" var="dept">
            	<option <c:if test="${map.prenatalReferral.mechanism == dept.id }">selected</c:if> value="${dept.id }">${dept.name }</option>
            </c:forEach>
            </select>
      	</div>
        ...
        <div id="u455" class="ax_下拉列表框">
            <select id="u455_input" name="department">
            	<c:forEach items="${roles }" var="role">
            		<option <c:if test="${map.prenatalReferral.department == role.id  }">selected</c:if> value="${role.id }">${role.name }</option>
            	</c:forEach>
            </select>
      	</div>
        ...
        <div id="u458" class="ax_下拉列表框">
            ...
            <input type="text" id="u458_input" name="nextFollowingDate" readonly 
        	style="color:#333333;cursor:pointer" onfocus="WdatePicker({dateFmt:'yyyyMMdd'})" 
        ...
        <div id="u461" class="ax_单选按钮">
            ...
            <input id="u461_input" <c:if test="${map.prenatalReferral.orderCheck == '1' }">checked="checked"</c:if> type="radio" value="1" name="orderCheck"/>
      	</div>
        <div id="u463" class="ax_单选按钮">
            <label for="u463_input">
                <p id="u464"><span> 否</span></p>
            </label>
            <input id="u463_input" <c:if test="${map.prenatalReferral.orderCheck == '0' }">checked="checked"</c:if> type="radio" value="0" name="orderCheck"/>
      	</div>
        ...
        <div id="u471" class="ax_下拉列表框">
            <select id="u471_input" name="healthHandbook" <c:if test="${map.prenatalReferral.healthHandbook == null }">class="selt"</c:if>>
              <option <c:if test="${map.prenatalReferral.healthHandbook == '1' }">selected</c:if> value="1">是</option>
              <option <c:if test="${map.prenatalReferral.healthHandbook == '0' }">selected</c:if> value="0">否</option>
            </select>
      	</div>
        <input type="hidden" id="prenatalId" name="prenatalId" value="${map.prenatalExa.id }"/>
        <div id="u467" class="ax_文本段落">
          <p id="u468"><span>叶酸发放：</span></p>
      	</div>
        <div id="u472" class="ax_下拉列表框">
            <select id="u472_input" onchange="only()" <c:if test="${map.prenatalReferral.folateNum == null }">class="selt"</c:if>>
              <option <c:if test="${map.prenatalReferral.folateNum != '0' && map.prenatalReferral.folateNum != null}">selected</c:if> value="1">是</option>
              <option <c:if test="${map.prenatalReferral.folateNum == '0' }">selected</c:if> value="0">否</option>
            </select>
      	</div>
        ...
        <div id="u473" class="ax_文本框">
        	<input id="u473_input" name="folateNum" type="text" placeholder="填写数量" value="${map.prenatalReferral.folateNum}"/>
      	</div>
     ...
     <div id="u07" class="ax_表格">
      ...
      <div id="u034" class="ax_形状" style="background-color:#E9854F">
          <p id="u035" onclick="window.location.href='${ctx}/highriskprojectmanagement/list?birthArchiveId=${map.birthArchives.id }&archivesId=${map.birthArchives.archivesId }'" style="cursor:pointer"><span>专案记录</span></p>
      </div>
