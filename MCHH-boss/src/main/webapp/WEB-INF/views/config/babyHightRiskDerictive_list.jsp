...
<html>
  <head>
    ...
    <script type="text/javascript">
		    var fid = '-1';
			var addUrl = "${ctx}/directive/addBabyHightRidk";
			var listUrl = "${ctx}/directive/listBabyHightRisk";
			var delUrl = "${ctx}/directive/del";
		    var updateViewUrl = "${ctx}/directive/updateBabyHightRiskView";
			    $(function() {
				...
				$.grid({id:'#dg',url:listUrl,title:"",toolbar:'#tb',
					columns:[[
		             ...
		             {title:'特殊疾病',field:'tgd',width : '200', resizable:'true',align:'center'},
		...
		<div style="position:relative; border: 1px solid #cccccc; top:-20px;height: 70px;width: 1120px;">
			...
			高危因素:
			 <select class="easyui-validatebox select" name="tgd" id="tgd" style="width:180px;border:1px solid #cccccc;">
				 		<option value="">不限</option>
					 <c:forEach var="HighRiskAll" items="${HighRiskAll}">
					 
					 	<option value="${HighRiskAll.id }">${HighRiskAll.name }</option>
					...
					<div  style="position:absolute;bottom: 400px" >
						  ...
						  <div id="u19" class="ax_文本框">
					        <select class="easyui-validatebox select" name="tgd" id="tg" style="width:180px;border:1px solid #cccccc;">
								 	<c:forEach var="special" items="${HighRiskAll }">
					      ...
					      <div id="u6" class="ax_形状">
					        <a href="javascript:void(0)" class="easyui-linkbutton" style="height: 35px; width: 80px; background-color: #FC7D45;border: 0px;color: white;position: absolute;left: 20%;bottom: 10%;" iconCls="icon-save" id="save" onclick="save()">保存</a>
					      </div>
