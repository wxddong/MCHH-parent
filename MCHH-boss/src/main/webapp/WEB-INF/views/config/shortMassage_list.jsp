...
<html class="panel-fit">
  <head>
	...
	<script type="text/javascript">
    	...
    	var listUrl = "${ctx}/shortmessage/list";
		var delUrl = "${ctx}/shortmessage/del";
	   	var updateUrl = "${ctx}/shortmessage/updateView";
		    $(function() {
		...
		function edit(node){
			...
			showDialog_URl('编辑预约检查短信模版','addOrUpdateAtv',updateUrl+'?id='+node.id,'moremiddle3');
		}
		
		//添加
		function add(){
			
			showDialog_('新建预约检查短信模版','addOrUpdateWin','moremiddle3');
		}
		//保存数据
		function save(){
		var addUrl = "${ctx}/shortmessage/add";
		...
		//修改
		function ed(){
			var update = "${ctx}/shortmessage/update";
	...
	<style type="text/css">
 	...
 	<body class="panel-noscroll">
		...
		<div style="position:relative; border: 1px solid #cccccc; top:-18px;height: 70px;width: 1120px;">
			...
			发送人群：
			<select class="easyui-validatebox select" name="sendCrowd" id="send"  style="width:180px;border:1px solid #cccccc;">
			 ...
			 可用标识：
			 <select class="easyui-validatebox select" name="useSign" id="Sign"  value="list.isvalid" style="width:180px;border:1px solid #cccccc;">
	   ...
	   <div id="addOrUpdateWin" class="easyui-dialog" title="预约检查" closed="true">
			...
			<form id="query" method="post">
				...
				<div  style="right:710px;bottom:420px;position:absolute;">
						<input type="hidden" name="messType" value="0"/>
					      ...
					      <div id="u14" class="ax_文本段落">
					          <p id="u15"><span id="u15">可用标识：</span></p>
					      </div>
					      ...
					      <div id="u19" class="ax_下拉列表框">
					        <select id="u19_input" name="sendTime" style="border:1px solid #cccccc;">
					        	<option></option>
					      ...
					      <div id="u4" class="ax_形状" style="background-color:#009dd9">
					          <p id="u5"><span id="cancel" onclick="can()">取消</span></p>
					      </div>
					
					      <!-- Unnamed (形状) -->
					      <div id="u6" class="ax_形状" style="background-color:#fc7d45">
					         <p id="u7"> <span id="save" onclick="save()">保存</span></p>
					      </div>
					        </div>
   					</div>
