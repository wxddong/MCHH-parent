...
<html class="panel-fit">
  <head>
	...
	<script type="text/javascript">
	    var fid = '-1';
	    var addUrl = "${ctx}/premaritalexainfo/toAdd";
		var listUrl = "${ctx}/archivesinfo/list";
		var delUrl = "${ctx}/acty/activity/deleteSign";
	    var updateUrl = "${ctx}/premaritalexainfo/toUpdate";
	    var delUrl = "${ctx}/archivesinfo/delWoman";
	    var doctorUrl = "${ctx}/archivesinfo/doctor";
	    var womanProveUrl = "${ctx}/premaritalexainfo/womanProve";
	    //详情
	    var detailUrl = "${ctx}/premaritalexainfo/detail";
    	   ...
    	   $.ajax({
			...
			$.grid({id:'#dg',url:listUrl,title:"",toolbar:'#tb',
				columns:[[
                 ...
                 {title:'姓名',field:'name',resizable:'true',width : '90',align:'center'},
                 ...
                 {title:'身份证号码',field:'idNo',resizable:'true',width : '160',align:'center'},
                 ...
                 {title:'对方姓名',field:'manName',resizable:'true',width : '90',align:'center'},
                 ...
                 {title:'婚检ID',field:'premaritalExaInfoPO.id',resizable:'true',width : '90',align:'center',hidden:true},
		...
		function detail(node){
			...
			var premaritalId = node.premaritalExaInfoPO.id;
			
			if (status == 'null' || status == '') {
				
				window.location=addUrl+'?id='+node.id+"&premaritalId="+premaritalId ;
			}else if (status == '0') {
				
				window.location = toUpdateUrl+'?id='+node.id+"&premaritalId="+premaritalId ;
			}
		}
		...
		function detailBao(node){
			...
			window.location=womanPresentationUrl+'?id='+node.id+"&premaritalId="+premaritalId ;
		...
		function detailMing(node){
			...
			window.location=womanProveUrl+'?id='+node.id ;
		...
		function detailList(node){
			...
			showDialog_URl('','detailAtv',detailUrl+'?id='+node.id,'dialList2');
		}
		//修改
		function edit(node){
		...
		<div id="tb" style="padding:5px;height:auto">
		 ...
		 <input id = "archId" name = "archId" type = "hidden" value="${archivesInfoDetail.archivesInfo.id}"/>
			 ...
			 <div style="position:relative;border: 1px solid #cccccc;height: 45px;width: 930px;">
				...
				<span style="position:absolute;left:120px ;width:60px; top:18px;height: 25px;">档案编号：</span> <input style="position:absolute;left:180px ;width:150px; top:15px;height: 25px;border: 1px solid #cccccc;"  class="easyui-validatebox text" type="text" name="archivesCode" id = "archivesCode" style="width:120px"/>
				<span style="position:absolute;left:350px ;width:40px; top:18px;height: 25px;">姓名：</span> <input style="position:absolute;left:390px ;width:90px; top:15px;height: 25px;border: 1px solid #cccccc;"  class="easyui-validatebox text" type="text" name="name" id = "name" style="width:120px"/>
				<span style="position:absolute;left:498px ;width:70px; top:18px;height: 25px;">身份证号：</span> <input style="position:absolute;left:560px ;width:180px; top:15px;height: 25px;border: 1px solid #cccccc;"  class="easyui-validatebox text" type="text" name="idNo" id = "idNo" style="width:120px"/><br><br>
				</div>
			...
			<div style="position: absolute;right:0%;top: 20px;height:45px;width:95px;">
      		...
      		<div style="position: absolute;right:0%;top: 140px;height:45px;width:95px;">
      			<a href="javascript:void(0)" class="easyui-linkbutton export" onClick="exportExcel('queryForm','womanPreExport');"><i class="fa fa-file-text-o"></i>导出</a>
      		</div>
    ...
    <div id="deleteAtv" class="easyui-dialog" title="角色" closed="true">
	      ...
	      <!-- Unnamed (形状) -->
      		<div id="u04" class="ax_形状" style="background-color:#009DD9;cursor:pointer">
