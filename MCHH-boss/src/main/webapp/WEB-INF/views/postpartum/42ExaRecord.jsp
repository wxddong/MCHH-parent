...
<!--    </div> -->
  ...
  <div id="42Record"> 
  	...
  	<div style="font-size: 18px;font-weight: 700;position: absolute;left: 550px;top: 90px;">产后42天健康检查记录表</div>
 ...
 <table border="1"  width="800" style="margin: auto;word-break:break-all; word-wrap:break-all;">
 		...
 		<tr>
 			...
 			<c:if test="${postpartumExaOther.vulva==1 }">外阴：${postpartumExaOther.vulvaRemark };</c:if>
          	...
          	<c:if test="${postpartumExaOther.other!='' && postpartumExaOther.other!=null}">其他：${postpartumExaOther.other}</c:if> </td>	
 		</tr>
 		 <tr>
 			<td align="center" style="font-size: 14px" height="50">分类</td>	
 			<td align="center" style="font-size: 14px"> <input type="radio" <c:if test="${postpartumExa.result==0 }">checked="checked"</c:if> disabled />1.已恢复  <input type="radio" <c:if test="${postpartumExa.result==1 }">checked="checked"</c:if> disabled />2.未恢复  
					<c:if test="${postpartumExa.result==1 }">
